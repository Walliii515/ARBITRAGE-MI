...
def _reverse_status(row: Dict, cfg: ReverseArbitrageConfig) -> str:
    ...
    if coverage is not None and coverage > cfg.orderbook_coverage_threshold:
...
def enrich_reverse_opportunities(
    ...
    for row in rows:
        ...
        basis_penalty_bps = max(float(basis_bps), 0.0) if basis_bps is not None else None
        net_edge_bps = None
        if borrow_24h_bps is not None and basis_penalty_bps is not None:
            net_edge_bps = (
                gross_funding_bps
                - borrow_24h_bps
                - full_fee_bps
        ...
        row.update({
            ...
            'reverse_basis_penalty_bps': round(basis_penalty_bps, 4) if basis_penalty_bps is not None else None,
            'reverse_net_edge_bps': round(net_edge_bps, 4) if net_edge_bps is not None else None,
            'reverse_open_basis_p20': threshold_meta.get('reverse_open_basis_p20'),
