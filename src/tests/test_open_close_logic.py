...
def make_trading_executor(sustain_sec=2.0, peak_pullback_pct=0.10,
                          ...
                          max_asset_exposure_ratio=0.20,
                          quality_scale_in_enabled=False,
                          quality_scale_in_enhanced_ratio=0.30,
...
class TestTradingExecutorFundingAdjustedEntry(unittest.TestCase):
    ...
    def test_default_concentration_limits_are_20_and_30_percent(self):
        te = make_trading_executor()

        self.assertEqual(te.max_asset_exposure_ratio, 0.20)
        self.assertEqual(te.quality_scale_in_enhanced_ratio, 0.30)

    def test_normal_concentration_allows_exactly_20_percent(self):
        te = make_trading_executor(
            quality_scale_in_enabled=False,
            vwap_threshold_meta={'ALLO': {'p20': 0.0}},
            close_vwap_threshold_meta={'ALLO': {'close_basis_p20': -100}},
        ...
        te._account_summary_ts = time.time()
        te._holding_spot_amount_by_asset['ALLO'] = 18.0
    ...
    def test_normal_concentration_blocks_above_20_percent_without_quality(self):
        te = make_trading_executor(
            quality_scale_in_enabled=False,
            vwap_threshold_meta={'ALLO': {'p20': 0.0}},
            close_vwap_threshold_meta={'ALLO': {'close_basis_p20': -100}},
        ...
        self.assertFalse(te._pass_risk_check(row))
        self.assertIn('总资金20%=20.00USDT', te._get_risk_fail_reason(row))

    def test_quality_scale_in_allows_exposure_to_enhanced_30_percent_limit(self):
        te = make_trading_executor(
            max_asset_exposure_ratio=0.20,
            quality_scale_in_enabled=True,
            quality_scale_in_enhanced_ratio=0.30,
        ...
        self.assertIn('20%->30%', row.get('_quality_scale_in_reason', ''))

    def test_quality_scale_in_rejects_above_30_percent_limit(self):
        te = make_trading_executor(
            max_asset_exposure_ratio=0.20,
            quality_scale_in_enabled=True,
            quality_scale_in_enhanced_ratio=0.30,
        ...
        te._account_summary_ts = time.time()
        te._holding_spot_amount_by_asset['ALLO'] = 29.0
        te._holding_weighted_basis_by_asset['ALLO'] = 20.0
        ...
        self.assertFalse(te._pass_risk_check(row))
        reason = te._get_risk_fail_reason(row)
        self.assertIn('增强30%=30.00', reason)
        self.assertIn('优质加仓拒绝', reason)

    def test_high_basis_quality_scale_in_allows_weak_funding_to_30_percent_limit(self):
        te = make_trading_executor(
            min_funding_rate_bps=25.0,
            min_funding_support_bps=8.0,
            realtime_min_funding_rate_bps=5.0,
...
class TestClosingExecutorFundingAwareClose(unittest.TestCase):
    ...
    def test_negative_funding_projected_loss_triggers_at_exact_threshold(self):
        self.pos.update({
            'open_spread_bps': 60.0,
            'current_spread_bps': 70.0,
            'funding_pnl_bps': -8.0,
            'funding_rate_24h': -0.0021,
            'funding_rate_sum_bps': 7.0,
        })

        self.assertEqual(self.ce._projected_exit_loss_bps(self.pos, 70.0), 40.0)
        self.assertEqual(
            self.ce._negative_funding_exit_mode(self.pos, 70.0),
            'projected_loss',
        )
        self.assertTrue(self.ce._check_negative_funding_exit(self.pos, 70.0))

    def test_negative_funding_projected_loss_below_threshold_remains_watch(self):
        self.pos.update({
            'open_spread_bps': 60.0,
            'current_spread_bps': 69.9,
            'funding_pnl_bps': -8.0,
            'funding_rate_24h': -0.0021,
            'funding_rate_sum_bps': 7.0,
        })

        self.assertAlmostEqual(self.ce._projected_exit_loss_bps(self.pos, 69.9), 39.9)
        self.assertIsNone(self.ce._negative_funding_exit_mode(self.pos, 69.9))
        self.assertEqual(self.ce._negative_funding_state(self.pos, 69.9), 'watch')

    def test_negative_funding_projected_loss_stops_for_recovered_positive_rate(self):
        self.pos.update({
            'open_spread_bps': 60.0,
    ...
    def test_negative_funding_projected_loss_uses_net_funding_pnl(self):
        self.pos.update({
            'open_spread_bps': 60.0,
            'current_spread_bps': 75.0,
            'funding_pnl_bps': 10.0,
            'funding_rate_24h': -0.0021,
            'funding_rate_sum_bps': 12.0,
        })
    ...
    def test_negative_funding_exit_detail_exposes_loss_composition(self):
        self.pos.update({
            'open_spread_bps': 60.0,
            'current_spread_bps': 75.0,
            'funding_pnl_bps': -8.0,
        ...
        self.assertIn('mode=projected_loss', detail)
        self.assertIn('预计退出损失45.0>=40.0bps', detail)
        self.assertIn('组成(价差-15.0+净资金费-8.0-22.0费)', detail)

    def test_negative_funding_projected_loss_recheck_cancels_after_vwap_improves(self):
        pos = dict(self.pos)
        pos.update({
            'status': 'holding',
            'future_contract': 'BTC_USDT',
        ...
        with (
            ...
            patch.object(
                self.ce,
                '_pre_execution_gate',
                return_value=(True, {'fresh': 'row'}, 65.0, ''),
            ),
            patch.object(self.ce, '_execute_close', execute_mock),
        ):
            results = self.ce.check_and_close([pos], {}, {'BTC': {'old': 'row'}})
    ...
    def test_negative_funding_projected_loss_recheck_executes_with_latest_vwap(self):
        pos = dict(self.pos)
        pos.update({
            'status': 'holding',
            'future_contract': 'BTC_USDT',
        ...
        with (
            ...
            patch.object(
                self.ce,
                '_pre_execution_gate',
                return_value=(True, {'fresh': 'row'}, 75.0, ''),
            ),
            patch.object(self.ce, '_execute_close', execute_mock),
        ):
            results = self.ce.check_and_close([pos], {}, {'BTC': {'old': 'row'}})
        ...
        self.assertIn('mode=projected_loss', detail)
        self.assertIn('预计退出损失45.0>=40.0bps', detail)
        self.assertIn('旁路✓(gate=75.0bps', detail)
        self.assertIsNone(execute_mock.call_args.kwargs['future_protective_price'])

    def test_negative_funding_projected_loss_requires_fresh_vwap(self):
        pos = dict(self.pos)
        pos.update({
            'status': 'holding',
            'future_contract': 'BTC_USDT',
